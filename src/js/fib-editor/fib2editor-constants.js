export const interactionReferenceString = 'http://www.comprodls.com/m1.0/interaction/fib2';

export const constantInputClass = {
    DOM_SEL_INPUT_BOX: 'userAnswer'
};

export const feedbackPresets = [ { key: 'correct', value: 'Show when Correct', showDropdown: true, order: 1 },
                           { key: 'incorrect', value: 'Show when Incorrect', showDropdown: true, order: 2 },
                           { key: 'generic', value: 'Show Always', showDropdown: true, order: 100 }];
